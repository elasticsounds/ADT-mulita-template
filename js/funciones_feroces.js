...
if (actividad === undefined){
  ...
  } else if (actividad == 182) {
      opciones.opcion = "enganio";
  }  else if (actividad == 183) {
...
localStorage.setItem('act',actividad);	
  ...
  const listaError = document.querySelectorAll('.ul-list li')

  listaError.forEach(ele => {
  ...
  if (ele.getAttribute('ischecked') === "false"){	
    ...
    ele.checked = false;
    document.querySelector("label[for='"+ele.id+"']").parentNode.classList.remove('wrong-answer');
    ele.setAttribute('ischecked', 'false') ;			
  }		
